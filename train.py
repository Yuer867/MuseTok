...
def train_model(epoch, model, dloader, dloader_val, optim, sched, use_attr_cls, val_rounds):
  model.train()
  ...
  for batch_idx, batch_samples in enumerate(dloader):
    ...
    batch_inp_time_sig = batch_samples['time_sig']
    batch_note_density = batch_samples['note_density']
    ...
    if not constant_kl:
      ...
      kl_beta = kl_max_beta
    batch_note_density = torch.ger(batch_note_density, torch.ones(config['data']['dec_seqlen'])).view(-1).to(device)
    losses = model.compute_loss(mu, logvar, kl_beta, free_bit_lambda, dec_logits, batch_dec_tgt, weighted_loss=True, weight=batch_note_density)
    ...
    # print (' -- epoch {:03d} | batch {:03d}: len: {}\n\t * loss = (RC: {:.4f} | KL: {:.4f} | KL_raw: {:.4f}), acc = {}, step = {}, beta: {:.4f} time_elapsed = {:.2f} secs'.format(
    #   epoch, batch_idx, batch_inp_time_sig, recons_loss_ema, kl_loss_ema, kl_raw_ema, acc, trained_steps, kl_beta, time.time() - st
    # ))
    
    print (' -- epoch {:03d} | batch {:03d}: \n\t * loss = (RC: {:.4f} | KL: {:.4f} | KL_raw: {:.4f}), acc = {}, step = {}, beta: {:.4f} time_elapsed = {:.2f} secs'.format(
      epoch, batch_idx, recons_loss_ema, kl_loss_ema, kl_raw_ema, acc, trained_steps, kl_beta, time.time() - st
    ))
    
    wandb.log({f"recons_loss": recons_loss_ema}, step=trained_steps)
    wandb.log({f"kl_loss": kl_loss_ema}, step=trained_steps)
    ...
    if not trained_steps % val_interval:
      vallosses = validate(model, dloader_val, n_rounds=val_rounds, use_attr_cls=use_attr_cls)
      with open(os.path.join(ckpt_dir, 'valloss.txt'), 'a') as f:
        f.write('[step {}] RC: {:.4f} | KL: {:.4f} | ACC: {:.4f} [val] | RC: {:.4f} | KL: {:.4f} | ACC: {:.4f}\n'.format(
          trained_steps, 
          recons_loss_ema, 
          kl_raw_ema,
          acc,
      ...
      wandb.log({f"kl_loss_val": np.mean(vallosses[1])}, step=trained_steps)
...
def validate(model, dloader, n_rounds=8, use_attr_cls=True):
  ...
  return loss_rec, kl_loss_rec, acc_all
...
if __name__ == "__main__":
  dset = REMIFullSongTransformerDataset(
    ...
    balanced_data=config['data']['balanced_data'],
  ...
  print('[info] use_attr_cls =', mconf['use_attr_cls'])
  ...
  run = wandb.init(
        ...
        resume="allow", project='MMP', group='', name='', id='reconstruct-pdmx-weighted-1')

  for ep in range(config['training']['max_epochs']):
    train_model(ep+1, model, dloader, dloader_val, optimizer, scheduler, mconf['use_attr_cls'], val_rounds=config['training']['val_rounds'])
