...
class MuseMorphose(nn.Module):
  ...
  def get_sampled_latent(self, inp, padding_mask=None, use_sampling=False, sampling_var=0.):
    # [shape of enc_inp] (seqlen_per_bar, n_bars_per_sample, d_embed)
    token_emb = self.token_emb(inp)
    enc_inp = self.emb_dropout(token_emb) + self.pe(inp.size(0))
    print(enc_inp.shape)

    # [shape of vae_latent] (n_bars_per_sample, d_latent)
  ...
  def forward(self, enc_inp, dec_inp, dec_inp_bar_pos, rfreq_cls=None, polyph_cls=None, padding_mask=None):
    ...
    print('enc_token_emb', enc_token_emb.shape)
    ...
    print('dec_token_emb', dec_token_emb.shape)
  ...
  def compute_loss(self, mu, logvar, beta, fb_lambda, dec_logits, dec_tgt, weighted_loss=False, weight=None):
...
if __name__ == "__main__":
    ...
    model = MuseMorphose(
      ...
      is_training=True, use_attr_cls=False).to(device)
    model.train()
    ...
    # x = torch.rand(1280, 16, 16)
    _, _, logits = model(enc_inp, dec_inp, dec_inp_bar_pos, padding_mask=padding_mask)
    print(logits.shape)
    # quantized, indices, commit_loss = residual_sim_vq(x)  # (1, 1024, 512), (1, 1024, 4), (1, 4)
